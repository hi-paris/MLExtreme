...
class Angular_bin:
    r"""Building blocks of angular minimum-volume sets (mvsets)
    ...
    \\prod_{i \\in \\{1, \\ldots, d\\}, i \\neq i_0} \\left[ \\frac{j_i}{J},
    \\frac{j_i+1}{J} \\right) \\times \\{e_{i_0}\\}

    where \(j_i \\in \\{0, \\ldots, J-1 \\} \), \( J \) is the chosen
    number of basic subdivisions of the interval \([0, 1)\) and \( i_0
    \) is the index of the canonical basis vector \( e_{i_0} \) normal
    to the face where the bin is located.

    Internallly a bin is encoded as a np.array of integers in \(0,
    \ldots, J-1\) of dimension d, containing the lower end points of
    the intervals in the product, except for the \( i_0^{th}\) entry
    which is equal to \( J\). A hash function is provided to enable
    quick responses to queries about the existence of keys in a
    dictionary of `angular_bin` class instances.
...
def sort_bins_by_mass(X: np.array,
                      J: int) -> list[Angular_bin]:
    bin_dict = {}
    n = np.shape(X)[0]
    #pdb.set_trace()
    for i in range(n):
...
class Xmvset:
    ...
    def predict(self, X_test, alpha):  # , standardize=True):
        ...
        set_bins_mvset = {Angular_bin(self.coords[i])
                          for i in range(id_mvset + 1)}
        #pdb.set_trace()
        ...
        for i in range(n_extremes_test):
            is_not_member[i] = (get_angular_bin(X_test_extreme[i], self.J)
                            not in set_bins_mvset)
    ...
    def score(self, X_test, angular_only=False):
        ...
        bin_dict = {Angular_bin(self.coords[i]): self.masses[i]
