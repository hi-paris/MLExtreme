...
class damex:
    """
    An unsupervised feature clustering model to learn "which subsets of features of a random vector
    are likely to be simultaneously large, while the others are small".

    This class provides methods to fit the model to data, calculate deviance, compute the Akaike
    Information Criterion (AIC), select optimal epsilon values based on AIC or cross-validation,
    and calculate the deviance between estimated and true subfaces.
    ...
    include_singletons : bool, default=False
        Whether to include singletons in training and testing.
    ...
    def select_epsilon_AIC(
        self,
        grid,
        X,
        standardize=True,
        unstable_eps_max=0.05,
        plot=False,
        ...
        -----------
        grid : list
            Grid of epsilon values to test.
        X : np.ndarray
            Input data.
        standardize : bool, default=True
            Whether to standardize the data.
        unstable_eps_max : float, default=0.05
            Maximum epsilon value for unstable solutions.
        plot : bool, default=False
            Whether to plot the AIC values.
        update_epsilon : bool, default=False
            Whether to update the model's epsilon value.

        Returns:
        --------
        eps_select_aic : float
        ...
        if plot:
            print(
                f"DAMEX: selected epsilon with AIC criterion "
            ...
            plt.plot([eps_select_aic, eps_select_aic], [0, max(vect_aic)], c="red")
            plt.grid(True)
            plt.show()
    ...
    def select_epsilon_CV(
        ...
        deviance_tol_cv : float
            Deviance value for the selected epsilon.
        ...
        for i, eps in enumerate(grid):
            cv_scores = self.deviance_CV(Xt, False, eps, thresh_test, cv, random_state)
        ...
        if plot:
            print(
                f"DAMEX: selected epsilon with CV criterion {round_signif(tol_cv, 2)}"
            )
    ...
    def deviance_to_true(self, subfaces_true, weights_true):
        ...
        Subfaces_matrix = ut.subfaces_list_to_matrix(Subfaces, self.dimension)
