...
# not needed for running clef/damex, only used here for visualisation
...
clustering = mlx.damex(min_counts=0,
...
"""
For this sample size, AIC and CV appear to be equally reliable to
estimate the expected deviance on a test set (or the deviance on a
asufficiently large test set as here) Additionally, the deviance
...
# Here and below we
# propose choosing epsilon in DAMEX using the AIC criterion. A
# CV-based selection rule is also implemented (but here we retain the
# AIC slection rule in the end)
...
eps_select_AIC, aic_opt, aic_values = clustering.select_epsilon_AIC(
    eps_vect, X, plot=False, update_epsilon=False)
...
eps_select_CV, _, _ = clustering.select_epsilon_CV(eps_vect, X, plot=False,
...
for i in range(neps):
    clust = mlx.damex(epsilon=eps_vect[i], min_counts=0, 
