...
plt.scatter(samples[:, 0], samples[:, 1], color='grey', alpha=0.5, label='Generated Samples')
...
plt.title("Samples from a Mixture of Dirichlet Distributions (p=2, n=500, k=2)")
plt.xlabel("Component 1")
plt.ylabel("Component 2")
plt.grid(True)
plt.show()
...
Mu = np.array([[0.3, 0.5,  0.7], [0.7, 0.5,  0.3]])  # Means for the mixture (2D)
wei = np.array([0.3, 0.4,0.3])  # Weights for each component
lnu = np.log(np.array([20,7,4]))  # Log scale parameters for the mixture components
# inspect the angular density
# Plot the mixture density. For depth computations, no modes will give better results
...
plt.figure(figsize=(8, 6))
plt.scatter(X[:, 0], X[:, 1], color='grey', alpha=0.5)  # grey points with transparency
plt.xlabel('X1')
plt.ylabel('X2')
