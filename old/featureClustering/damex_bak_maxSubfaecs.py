...
class damex:
    ...
    def deviance(self, Xtest, thresh_test=None, standardize=False):
        ...
        Subfaces = self.subfaces  # self.maximal_subfaces if self.use_max_subfaces \
            # else self.subfaces
        Masses = self.masses  # self.maximal_masses if self.use_max_subfaces \
            # else self.masses

        negative_pseudo_lkl = ut.total_deviance(
            Subfaces, Masses, Xt, self.thresh_test,
            self.include_singletons, self.epsilon,
            rate=self.rate)
    ...
    def get_AIC(self, Xtrain,  standardize=True):
        ...
        Masses = self.masses
        # self.maximal_masses if self.use_max_subfaces \
        #     else self.masses
    ...
    def deviance_to_true(self, subfaces_true, weights_true):
        ...
        if not self.include_singletons:
            ...
            weights_true = weights_true / np.sum(weights_true) # !! MODIF
